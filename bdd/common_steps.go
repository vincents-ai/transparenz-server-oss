...
func RegisterCommonSteps(s *godog.ScenarioContext) {
	...
	s.Step(`^a running Greenbone test server with an authenticated organization$`, commonSetupServerWithOrg)
	s.Step(`^a running VEX server with an authenticated organization$`, commonSetupServerWithOrg)
	s.Step(`^a running server with an authenticated organization for telemetry$`, commonSetupServerWithOrg)
	...
	s.Step(`^the response should contain a plaintext secret$`, commonAssertSecret)
...
func commonAssertSecret() error {
	...
	if resp.Secret == "" {
		return fmt.Errorf("response does not contain a plaintext secret")
	}
	return nil
}
