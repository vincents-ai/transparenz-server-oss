...
// GreenboneAuthMiddleware returns a Gin middleware that authenticates Greenbone webhook requests.
func GreenboneAuthMiddleware(db *gorm.DB, updater *LastUsedAtUpdater) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token := c.GetHeader("X-Greenbone-Token")
		if token == "" {
			api.Unauthorized(c, "X-Greenbone-Token header is required")
			return
		}
		...
		c.Set("greenbone_org_id", webhook.OrgID)
		c.Set("greenbone_actions", webhook.Actions)
